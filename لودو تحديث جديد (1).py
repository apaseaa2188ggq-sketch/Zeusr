...
# ==================== SYSTEM EXPIRY ====================#
EXPIRY_DATE = datetime(2026, 9, 15)  # تاريخ انتهاء الصلاحية
...
{xp} TODAYS   {xpxx} {datetime.now().strftime('%d/%B/%Y')}
{xlinex}"""
...
def print_logo():
    try:
        output = render('Y A L L A', colors=['green', 'red'], align='center')
        print(output)
    except:
        pass
...
def show_countries():
    ...
    for key, country in COUNTRIES.items():
        ...
        print(f"{G}[{W}{key}{G}] {country['name']} {G}➜ {W}{country['code']} {G}➜ {W}{prefixes}")
    print(f"{G}{'='*50}{W}\n")
...
print(f"{xp} Selected: {G}{selected_country['name']}{W} | Code: {G}{COUNTRY_CODE}{W} | Prefixes: {G}{', '.join(PREFIXES)}{W}")
print(xlinex)
...
MAX_RUN_TIME = 7200  # 7200 ثانية = ساعتين
...
def check_number(mobile):
    global results, stats, total_checked, hit_accounts, stop_flag, START_TIME
    if stop_flag:
        return

    # التحقق من مرور ساعتين
...
def print_dashboard():
    ...
    print(f"\n{xp} COUNTRY    {xpxx} {G}{selected_country['name']}{W}")
    print(f"{xp} HIT     {xpxx} {G}{stats['good']}{W}    WRONG     {xpxx} {R}{stats['wrong_pass']}{W}    NOTREG     {xpxx} {Y}{stats['not_registered']}{W}")
    print(xlinex)
    print(f"{xp} HIT ACCOUNTS")
    if hit_accounts:
        for acc in hit_accounts[-10:]:
            print(f"{xp} {PINK}[HIT]{W} {PINK}{acc['phone']}{W}    {YELLOW}{acc['password']}{W}    ID: {GREEN}{acc['id']}{W}")
    else:
        print(f"{xp} No hits yet...")
    print(xlinex)
    print(f"{xp} TOTAL     {xpxx} {W}{stats['total']}")
    print(f"{xp} TIME LEFT {xpxx} {G}{hours:02d}:{minutes:02d}:{seconds:02d}{W}")
    print(f"{xp} STATU:    {xpxx} {G}SCANNING...{W}")
    print(xlinex)
...
def main():
    ...
    # رسالة انتهاء التفعيل بعد ساعتين
    print("\033[1;31m" + "="*56)
    print(EXIT_MESSAGE)
    print("="*56 + "\033[0m")
    ...
    elapsed = (datetime.now() - START_TIME).total_seconds()
    print(f"Time elapsed: {elapsed / 60:.2f} minutes ({elapsed / 3600:.2f} hours)")
